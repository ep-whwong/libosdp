...
static int cp_submit_command(struct osdp_pd *pd, const struct osdp_cmd *cmd) {
    struct osdp_cmd *p;
    uint32_t all_flags = (OSDP_CMD_FLAG_BROADCAST);
...
static int cp_detect_connection_topology(struct osdp *ctx) {
    ...
    struct osdp_pd *pd;
    static struct disjoint_set set;
...
static struct osdp_pd pd_array[2];
...
/* --- Exported Methods --- */
static struct osdp staticCtx[OSDP_CP_MAX] = {0, 0, 0, 0};

osdp_t *osdp_cp_setup(int num_pd, const osdp_pd_info_t *info) {
    struct osdp *ctx = NULL;
    ...
    if (num_pd == 0 || num_pd > OSDP_PD_MAX) {
        return NULL;
    }

    for (uint8_t i = 0; i < OSDP_CP_MAX; i++) {
        if (staticCtx[i]._magic != 0)
            continue;
        ctx = &staticCtx[i];
        break;
    }
